...
class PacmanDQN(game.Agent):
    ...
    def getMove(self, state):
        ...
        if np.random.rand() > self.params['eps']:
            ...
            self.Q_pred = self.qnet.sess.run(
                self.qnet.y,
                feed_dict = {self.qnet.x: np.reshape(self.current_state,(1, self.params['width'], self.params['height'], 6)),
    ...
    def observation_step(self, state):
        if self.last_action is not None:
            ...
            """
            开始分析 在 pacman 当中agent rules 与 ghost rules 当中关于 score 和 reward 的分析
            在 pacman.py 的consume当中，
            eat foot score = 10
            pacman wins the game score = 500
            pacman eats capsule score ??
            time pass score = TIME_PENALTY (值为1)
            agent collides with the ghost, if the scaredTimer >1, 则    agent 吃掉这个 scaredTimer, score = 200
                                                                  否则  ghost 吃掉这个 agent, score = -500
            eat capsule 呢？ 有奖励吗？ 答案是没有影响
            """
    ...
    def getStateMatrices(self, state):
        ...
        # print('---------------- observation shape before----------------')
